...
@section('title', 'Master Data Agama - Arsip Digital')
...
<div class="mb-5">
    <div class="row align-items-center">
        <div class="col-md-6">
            <h3 class="fw-extrabold mb-1 text-dark">Master Data Agama</h3>
            <p class="text-muted mb-0">Kelola informasi referensi agama dan distribusi pegawai.</p>
        </div>
        @if(session('role') == 'superadmin')
        <div class="col-md-6 text-md-end mt-3 mt-md-0">
            <button class="btn btn-gradient-primary shadow rounded-pill px-4 py-2" data-bs-toggle="modal" data-bs-target="#modalTambahAgama">
                <i class="bi bi-plus-circle-fill me-2"></i> Tambah Agama
            </button>
        </div>
        @endif
    </div>
...
<div class="card card-modern shadow-sm border-0 mb-4">
            ...
            <div class="row align-items-center g-3">
                <div class="col-md-6">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <h5 class="fw-bold mb-0">Referensi Agama</h5>
                            <small class="text-muted">{{ $agamas->count() }} agama terdaftar</small>
                        </div>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="search-wrapper position-relative">
                        ...
                        <input type="text" id="searchMainAgama" class="form-control rounded-pill ps-5 bg-light border-0" placeholder="Cari nama agama...">
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="card-body p-0 mt-3">
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0" id="tabelMainAgama">
                <thead>
                    <tr>
                ...
                <tbody>
                    @forelse($agamas as $index => $agama)
                        <tr>
                            ...
                            <td>
                                <div class="d-flex align-items-center">
                                    <div class="avatar-circle me-3 text-uppercase" style="background: #eef2ff; color: #4e73df; font-size: 12px;">
                                        {{ substr($agama->nama, 0, 1) }}
                                    </div>
                                    <span class="fw-bold text-dark">{{ $agama->nama }}</span>
                                </div>
                            </td>
                            ...
                            <td class="text-center pe-4">
                                @if(session('role') == 'superadmin')
                                <button class="btn btn-sm btn-outline-warning rounded-circle me-2" style="width: 32px; height: 32px; padding: 0;" data-bs-toggle="modal" data-bs-target="#modalEditAgama{{ $agama->id }}" title="Edit">
                                    <i class="bi bi-pencil-square"></i>
                                </button>
                                <form action="{{ url('/admin/master/agama', $agama->id) }}" method="POST" class="d-inline" onsubmit="confirmDeleteSweetAlert(this); return false;">
                                    @csrf @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center py-5 text-muted">Belum ada data agama.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
{{-- Modal Tambah Agama --}}
<div class="modal fade" id="modalTambahAgama" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4">
            ...
            <form action="{{ url('/admin/master/agama') }}" method="POST">
                @csrf
                <div class="modal-body px-4">
                    <label class="form-label fw-semibold small">Nama Agama</label>
                    <input type="text" class="form-control form-control-lg rounded-3 fs-6" name="nama" required placeholder="Contoh: Islam">
                </div>
...
{{-- Modal Edit --}}
<div class="modal fade" id="modalEditAgama{{ $agama->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4">
            <div class="modal-header border-0 pt-4 px-4">
            ...
            <form action="{{ url('/admin/master/agama/'.$agama->id) }}" method="POST">
                @csrf @method('PUT')
                <div class="modal-body px-4">
                    <label class="form-label fw-semibold small">Nama Agama</label>
                    <input type="text" class="form-control form-control-lg rounded-3 fs-6" name="nama" required value="{{ $agama->nama }}">
                </div>
                <div class="modal-footer border-0 pb-4 px-4">
...
{{-- Modal Detail ASN --}}
<div class="modal fade" id="modalDetailASN{{ $agama->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-md">
        <div class="modal-content border-0 shadow-lg rounded-4">
            <div class="modal-header modal-detail-header border-0 px-4 pt-4 pb-3">
                <div>
                    <h5 class="fw-bold mb-1">
                        <i class="bi bi-people-fill me-2"></i>Daftar ASN — {{ $agama->nama }}
                    </h5>
                    <small class="opacity-75">{{ $agama->pegawais_count }} pegawai terdaftar</small>
                </div>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body px-4 py-3" style="max-height: 420px; overflow-y: auto;">
                @if($agama->pegawais->isEmpty())
                    <div class="text-center py-4 text-muted">
                        <i class="bi bi-person-x display-4 text-light"></i>
                        <p class="mt-2 small">Belum ada ASN dengan agama ini.</p>
                    </div>
                @else
                    @foreach($agama->pegawais as $pegawai)
